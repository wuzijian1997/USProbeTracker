...
const std::string USSTREAMDISPLAYNAME
    = "Ultrasound Stream"; // Name of window that this application writes to

class USVideoStreaming
{
public:
    // Class Vars
    bool _showStream = false; // Whether we are printing the stream to an OpenCV window or not (this
                              // window can be shown to the patient instead of the scrcpy window)
    ...
    cv::Mat USFrameScaled, USFrame; // USFrameScaled is dpi scaled, then USFrame is rescaled to
                                    // match the proper display size

    // Set up the streaming of the US video
    // Create the streaming object
    explicit USVideoStreaming(
        bool show_stream, int timeout); // To Do: Allow Dynamic size changes on the scrcpy window
    ~USVideoStreaming();
