...
const double FAR_CLIP = 1.7f; // Average Arm Span is 65 cm, we want the tracker to be about the same
                              // distance from eyes to hand
const double DEFAULT_DEPTH_SCALE = 1000.0f; // Converts raw depth values to meters, must update the
                                            // value using the setDepthScale() method
...
public:
    ...
    int _timeout = 20; // Amount of time we wait for the thread to update a new frame before
    ...
    // Method to get the realsense data (ir frame left, ir frame right, depth frame, vector of depth
    // filtered, vector of left ir
    bool getRealSenseData(rs2::frame &ir_frame_left,
...
private:
    ...
    // Method to wait for new frame arrival, runs on separate thread, also converts data to type
    // expected by findKeyPoints() function
    void ReadFrames();
