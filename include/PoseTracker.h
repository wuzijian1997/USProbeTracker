...
const float MARKER_DIAMETER_ERROR_THRESHOLD
    = 0.01f; // Allowable difference between actual marker diam and measured
...
public:
    struct IRPose
    {
        IRPose() { pose = Eigen::Isometry3d::Identity(); }
    ...
    explicit PoseTracker(std::shared_ptr<IRSegmentation> irTracker,
    ...
    /// @param irIm Infrared reflectivity image for segmenting the markers. Ownership of the pointer
    /// passes to this function</param>
    /// @param depthMap Depth map for obtaining the depth value at the segmented locations.
    /// Ownership of the pointer passes to this function</param>
    void update(std::unique_ptr<std::vector<uint8_t>> irImLeft,
                std::unique_ptr<std::vector<uint8_t>> irImRight);

    /// Return the most recent computed pose of the tracked object
    Eigen::Matrix4d getPose();

    /// Return not only the marker pose, but also the positions of the individual spheres in world
    ...
    /// Return true if it has measured a pose that hasn't been fetched yet through getPose or
    /// getLastMeasurement
    bool hasNewPose();
    ...
    /// @param jumpThresholdMetres if a point's position changes more than this amount (in metres)
    /// between frames, it is considered to have jumped and is ignored
    /// @param numFramesUntilSet if a point jumped but is still there after this many frames, the
    /// new position is probably correct so we accept it
    void setJumpSettings(bool filterJumps,
    ...
    /// Thread the continuously looks for new detections in the images and then handles the further
    /// processing
...
private:
    struct SensorPacket
    {
        SensorPacket(std::unique_ptr<std::vector<uint8_t>> ir_left,
                     std::unique_ptr<std::vector<uint8_t>> ir_right)
    ...
    /// The marker points in local coordinates, in units of metres. Should be arranged so the
    /// centroid of the points is the origin
    ...
    int m_numJumpFrames = 3; // If the jump remains in place for more than this number of frames, we
                             // take it to be the new position
    ...
    double bruteForceMatch(const std::vector<Eigen::Vector3d> &mesPts,
                           std::vector<int> &idxsWithOutliers,
                           std::vector<int> &idxs,
                           bool useFixedIdxs);
    void poseCalcFailed();
    Eigen::Matrix3d kabsch(const std::vector<Eigen::Vector3d> &mes,
    ...
    void setPoseFromResult(const std::shared_ptr<IRSegmentation::IrDetection> mes,
                           const std::vector<int> idxs);
    Eigen::Isometry3d transformFromIdxs(const std::vector<Eigen::Vector3d> &mes,
                                        const std::vector<int> &idxs);
    Eigen::Isometry3d findTransformFromPoints(const std::vector<Eigen::Vector3d> &mes,
                                              const std::vector<Eigen::Vector3d> &exp);
