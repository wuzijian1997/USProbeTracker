...
const double FAR_CLIP = 1.3f;
// Average Arm Span is 65 cm, we want the tracker to be about the same distance from eyes to hand
const double EPIPOLAR_MATCH_Y_THRESHOLD = 4.0f;
// Difference in y-direction for two points to be in same epipolar line for stereo triangulation
const double EPIPOLAR_MATCH_X_THRESHOLD = 75.0f;
// Difference in x-direction for two points to be the same in matching epipolar points
const int UNDISTORTION_MAX_COUNT = 40; // Number of times undistortPoints() iterates
const double UNDISTORTION_EPS = 1e-12;
// Threshold of undistortion accuracy (either this or the max iterations are met)
...
public:
    ...
    enum class DetectionMode { Contour, Blob };

    enum class LogLevel { Silent, Verbose, VeryVerbose };

    /// Create an IrTracker object.
    /// @param width the width of the input image in pixels
    /// @param height the height of the input image in pixels
    IRSegmentation(int width,
                   int height,
                   double depth_scale,
                   bool show_clip_area,
    ...
    void setCameraBoundaries(int roiUpperRow,
                             int roiLowerRow,
                             int roiLeftCol,
                             int roiRightCol,
    ...
    /// Register a function which is used to compute the camera intrinsics. It should take a point
    /// in the image coordinates and map it to the camera unit plane
    /// @param uv the (column, row) coordinates of a point in the image, in pixels
    /// @param xy This array is filled in when the function is called. The point is at position
    /// depth * (xy[0], xy[1], 1) in the camera frame
    ...
    std::shared_ptr<IrDetection> findKeypointsWorldFrame(
    ...
    cv::Mat _R, _T, _E, _F;
    cv::Mat _R_left, _R_left_inv, _R_right;
    // Transforms points from unrectified camera coordinate system to rectified coordinate system
    // for each camera
    cv::Mat _P_left, _P_right;
    // Transforms points in rectified camera coordinate system to camera's recitified image
...
private:
    std::function<void(const std::array<double, 2> &, std::array<double, 2> &)>
        m_imagePointToCameraUnitPlane;
