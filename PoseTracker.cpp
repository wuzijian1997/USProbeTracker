...
bool PoseTracker::tryEuclideanMatching(const std::vector<Eigen::Vector3d>& mesPts, std::vector<int>& idxs)
{
...
std::vector<int> PoseTracker::euclideanVotingOutliers(const std::vector<Eigen::Vector3d>& mesPts)
{
    ...
    Eigen::MatrixXd D; D.setZero(np, np);
    ...
    // Voting matrix
    Eigen::MatrixXd V; V.setZero(mp, np);
...
double PoseTracker::bruteForceMatch(const std::vector<Eigen::Vector3d>& mesPts, std::vector<int>& idxsWithOutliers, std::vector<int>& idxs, bool useFixedIdxs)
{
    size_t np = mesPts.size();
...
Eigen::Isometry3d PoseTracker::transformFromIdxs(const std::vector<Eigen::Vector3d>& mes, const std::vector<int>& idxs)
{
    std::vector<Eigen::Vector3d> mesPts;
    std::vector<Eigen::Vector3d> expPts;
    for (size_t i = 0; i < idxs.size(); i++)
...
Eigen::Isometry3d PoseTracker::findTransformFromPoints(const std::vector<Eigen::Vector3d>& mes, const std::vector<Eigen::Vector3d>& exp)
{
...
Eigen::Matrix3d PoseTracker::kabsch(const std::vector<Eigen::Vector3d>& mes, const std::vector<Eigen::Vector3d>& exp)
{
    ...
    // Create matrices
    Eigen::MatrixXd At; At.setZero(3, n); // 3 x n
    Eigen::MatrixXd B;  B.setZero(n, 3);  // n x 3
...
{
    // Calculate SVD
    auto jacobiSvd = Eigen::JacobiSVD<Eigen::Matrix3d>(mat, Eigen::ComputeFullV | Eigen::ComputeFullU);
...
double PoseTracker::meanMatchError(const std::vector<Eigen::Vector3d>& mesPts, const std::vector<int>& idxs)
{
    // Make the lists of points in the new order
    std::vector<Eigen::Vector3d> exp;
    std::vector<Eigen::Vector3d> mes;
...
Eigen::Quaterniond PoseTracker::slerp(const Eigen::Quaterniond& q1, const Eigen::Quaterniond& q2, const float& t)
{
