...
bool RealSense::RealSenseInit(int width,
                              int height,
                              int fps,
                              float enable_laser,
                              float laser_power,
                              ...
                              float exposure_level)
{
    ...
    auto depth_units = _realSense_context.query_devices().front().query_sensors().front().get_option(
    ...
    _realSense_intrinsics_leftIR = pipeline_profile.get_stream(RS2_STREAM_INFRARED, 1)
                                       .as<rs2::video_stream_profile>()
                                       .get_intrinsics();
    _realSense_intrinsics_rightIR = pipeline_profile.get_stream(RS2_STREAM_INFRARED, 2)
                                        .as<rs2::video_stream_profile>()
                                        .get_intrinsics();
    ...
    for (auto &sensor : sensors) {
        ...
        if (sensor.supports(RS2_OPTION_ENABLE_AUTO_EXPOSURE)
            ...
            sensor.set_option(RS2_OPTION_ENABLE_AUTO_EXPOSURE,
                              enable_autoexposure); // Turn OFF auto exposure
            std::cout << "Auto exposure set to " << enable_autoexposure
                      << " for sensor: " << sensor.get_info(RS2_CAMERA_INFO_NAME) << std::endl;
        }

        // Enable auto exposure for BGR
        if (sensor.supports(RS2_OPTION_ENABLE_AUTO_EXPOSURE)
            && sensor.get_info(RS2_CAMERA_INFO_NAME) == std::string("RGB Camera")) {
            sensor.set_option(RS2_OPTION_ENABLE_AUTO_EXPOSURE,
                              REALSENSE_BGR_AUTOEXPOSURE_ENABLE); // Turn OFF auto exposure
            std::cout << "Auto exposure set to " << REALSENSE_BGR_AUTOEXPOSURE_ENABLE
                      << " for sensor: " << sensor.get_info(RS2_CAMERA_INFO_NAME) << std::endl;
        }
        ...
        if (sensor.supports(RS2_OPTION_EXPOSURE)
            ...
            std::cout << "Exposure set to " << exposure_level
                      << " for sensor: " << sensor.get_info(RS2_CAMERA_INFO_NAME) << std::endl;
        }
    ...
    if (depth_sensor.supports(RS2_OPTION_EMITTER_ENABLED)) {
        depth_sensor.set_option(RS2_OPTION_EMITTER_ENABLED,
                                enable_laser); // Enable or disable emitter
    }
    ...
    while (_isRealSenseRunThread) {
        try {
...

            {
                ...
                // Clears the queue before pushing the newest reading (we only have one reading in
                // the queue at a time)
                while (!_realSenseDataQueue.empty()) {
                    _realSenseDataQueue.pop();
                }
...
bool RealSense::getRealSenseData(RealSenseData &realsense_data)
{
    ...
    if (!_realSenseFrameArrivedVar.wait_for(lock, std::chrono::milliseconds(_timeout), [this]() {
            return !_realSenseDataQueue.empty();
        })) {
