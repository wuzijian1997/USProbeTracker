...
IRSegmentation::IRSegmentation(int width,
                               int height,
                               double depth_scale,
                               bool show_clip_area,
    ...
    , _F(F)
{
    ...
    cv::Mat Q;
    cv::stereoRectify(_left_camera_mat,
                      _left_dist,
                      _right_camera_mat,
                      _right_dist,
    ...
    // Takes the inverse of the rotation matrix moving the camera coordinate to the rectified
    // coordinate system
    ...
    m_imagePointToCameraUnitPlane = [width, height](const std::array<double, 2> &uv,
...
Eigen::Vector4i IRSegmentation::setROI(int xMin, int xMax, int yMin, int yMax)
{
    ...
    if (xMin < m_depthCamRoiLeftCol)
        xMin = m_depthCamRoiLeftCol;
    if (yMin < m_depthCamRoiUpperRow)
        yMin = m_depthCamRoiUpperRow;
    if (xMax > m_depthCamRoiRightCol)
        xMax = m_depthCamRoiRightCol;
    if (yMax > m_depthCamRoiLowerRow)
        yMax = m_depthCamRoiLowerRow;
...
void IRSegmentation::setCameraBoundaries(int roiUpperRow,
                                         int roiLowerRow,
                                         int roiLeftCol,
                                         int roiRightCol,
                                         ...
                                         double farClipPlane)
{
    ...
    if (m_xMaxCrop == m_imWidth - 1 && m_yMaxCrop == m_imHeight - 1 && m_xMinCrop == 0
        && m_yMinCrop == 0) {
        setROI(m_xMinCrop, m_xMaxCrop, m_yMinCrop, m_yMaxCrop);
    }

    spdlog::info("Set camera boundaries to upper row {}, lower row {}, left col {}, right col {}, "
                 "near clip {}, far clip {}",
                 m_depthCamRoiUpperRow,
                 m_depthCamRoiLowerRow,
                 m_depthCamRoiLeftCol,
                 m_depthCamRoiRightCol,
                 m_depthNearClip,
                 m_depthFarClip);
}

void IRSegmentation::setCameraIntrinsics(
    std::function<void(const std::array<double, 2> &, std::array<double, 2> &)> intrinsics)
{
...
std::shared_ptr<IRSegmentation::IrDetection> IRSegmentation::findKeypointsWorldFrame(
    std::unique_ptr<std::vector<uint8_t>> irImLeft, std::unique_ptr<std::vector<uint8_t>> irImRight)
{
    ...
    if (success_left && success_right) {
        ...
        if (m_show_clip_area) {
            ...
            cv::drawKeypoints(imLeft,
                              keypoints_left,
                              outputImage_left,
                              cv::Scalar(0, 255, 0),
                              cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
            cv::drawKeypoints(imRight,
                              keypoints_right,
                              outputImage_right,
                              cv::Scalar(0, 0, 255),
                              cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
        ...
        //*********Step 1: Get 2D Keypoint Location in Original Left/Right (uncropped) image
        //planes*************
        std::vector<cv::Point2f> left_points, right_points;
        std::vector<int> valid_keypoints_left_indices;
        // Tracks indices of valid keypoints (ones that aren't ouside the left camera sensor)
        ...
        for (size_t i = 0; i < keypoints_left.size(); i++) {
            ...
            if (y < m_depthCamRoiUpperRow || y > m_depthCamRoiLowerRow || x < m_depthCamRoiLeftCol
                || x > m_depthCamRoiRightCol) {
        ...
        //******************Step 2: Undistort the keypoints in the left/right
        //images*********************** It projects them onto rectified image planes
        std::vector<cv::Point2f> left_undistorted, right_undistorted;

        cv::undistortPoints(left_points,
                            left_undistorted,
                            _left_camera_mat,
                            _left_dist,
                            _R_left,
                            ...
                            cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS,
                                             UNDISTORTION_MAX_COUNT,
                                             UNDISTORTION_EPS));
        cv::undistortPoints(right_points,
                            right_undistorted,
                            _right_camera_mat,
                            _right_dist,
                            _R_right,
        ...
        // //For a rectified image
        triangulatePointsIterativeLinear(_P_left,
                                         _P_right,
                                         left_matched,
                                         right_matched,
                                         left_cam_points4d);
        // triangulatePointsIterativeEigen(_P_left, _P_right, left_matched, right_matched,
        // left_cam_points4d);
        ...
        for (int i = 0; i < left_cam_points3D_rect.rows; i++) {
            ...
            double keypoint_diameter_px
                = keypoints_left[valid_keypoints_left_indices[valid_left_point_indices[i]]]
                      .size; // Get pixel diameter in pixels

            // Gets the 2D positions of left/right edges in the left image plane
            double x_left_edge_px = left_points[valid_left_point_indices[i]].x
                                    - keypoint_diameter_px / 2;
            double x_right_edge_px = left_points[valid_left_point_indices[i]].x
                                     + keypoint_diameter_px / 2;
            ...
            //*******************Project the X,Y,Z points onto the left camera
            //plane****************** Don't actually need to project points because I already have
            // that info in the left camera (left_points) std::vector<cv::Point3f> objectPoints = {
            // cv::Point3f(X, Y, Z) }; // 3D point in left camera coordinate system std::vector<cv::Point2f>
            // projectedPoints; // 2D point in left image plane
...
bool IRSegmentation::contourDetect(cv::Mat &im, std::vector<cv::KeyPoint> &keypoints)
{
    ...
    for (size_t i = 0; i < contours.size(); i++) {
        ...
        if (_area < COUNTOUR_MIN_AREA || _area > COUNTOUR_MAX_AREA || _cvxity < CONTOUR_CONVEXITY
            || _circy < COUNTOUR_CIRCULARITY)
            continue;
...
{
    float assumed_depth = 0.9f;
    // Assume a fixed distance so we can find a point along the epipolar line

    // Transform each corner to right image
    cv::Point2f topLeftR = transformPointToRight(left_ROI.x, left_ROI.y, assumed_depth);
    cv::Point2f bottomRightR = transformPointToRight(left_ROI.x + left_ROI.width,
                                                     left_ROI.y + left_ROI.height,
                                                     assumed_depth);

    // Create the corresponding ROI in the right image
    right_ROI = cv::Rect(topLeftR.x,
                         topLeftR.y,
                         bottomRightR.x - topLeftR.x,
                         bottomRightR.y - topLeftR.y);
}

cv::Point2f IRSegmentation::transformPointToRight(int left_point_x,
                                                  int left_point_y,
                                                  float assumed_depth)
{
