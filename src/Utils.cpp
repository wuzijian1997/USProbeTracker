...
std::string calculateForceVals(std::string &raw_force_string,
                               ...
                               Eigen::MatrixXd &force_compensation_mat)
{
    ...
    if (raw_force_string.find("NaN") != std::string::npos) {
        ...
        Eigen::Vector3d force_xyz = calib_mat
                                    * raw_force_vector; // .transpose(); Transpose here is wrong
...
// These triangulation methods come from:
// https://github.com/PavelNajman/Triangulation/blob/master/IterativeLS.cpp
//  Iterative Midpoint Triangulation Function
void triangulatePointsIterativeLinear(const cv::Mat &P0,
                                      const cv::Mat &P1,
...
void triangulatePointsIterativeEigen(const cv::Mat &P0,
                                     const cv::Mat &P1,
                                     const std::vector<cv::Point2f> &left_matched,
                                     const std::vector<cv::Point2f> &right_matched,
                                     cv::Mat &points4D)
