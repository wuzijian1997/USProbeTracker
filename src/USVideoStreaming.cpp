    ...
    , _timeout(timeout)
{
    ...
    USFrame.create(_windowHeight_original,
                   _windowWidth_original,
                   CV_8UC3); // Frame that is stored with actual size
...
void USVideoStreaming::ReadFrames()
{
    while (_runFrame_Thread) {
        ...
        // StretchBlt(_hwindowCompatibleDC, 0, 0, _windowWidth, _windowHeight, _hwindowDC, 0, 0,
        // _windowWidth, _windowHeight, SRCCOPY);
        BitBlt(_hwindowCompatibleDC, 0, 0, _windowWidth, _windowHeight, _hwindowDC, 0, 0, SRCCOPY);
        GetDIBits(_hwindowCompatibleDC,
                  _hbwindow,
                  0,
                  _windowHeight,
                  USFrameScaled.data,
        ...
        if (!USFrameScaled.empty())
            cv::resize(USFrameScaled,
                       USFrame,
                       cv::Size(_windowWidth_original, _windowHeight_original),
                       0,
                       0,
                       cv::INTER_CUBIC);
...
cv::Mat USVideoStreaming::getFrame()
{
    ...
    if (!_frameArrivedVar.wait_for(lock, std::chrono::milliseconds(_timeout), [this]() {
