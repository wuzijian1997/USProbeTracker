...
PoseTracker::PoseTracker(std::shared_ptr<IRSegmentation> irTracker,
    ...
    , m_markerDiameter(markerDiameter)
{
    ...
    m_width = REALSENSE_WIDTH;
    m_height = REALSENSE_HEIGHT;
    m_roi = {0, m_width - 1, 0, m_height - 1};

    // Create lists of permutations for brute force matching //Change if we are using more markers
    ...
    for (int i = 0; i < m_nPoints; i++) {
        if (m_logLevel == IRSegmentation::LogLevel::VeryVerbose)
            LOG << "Geom: " << m_markerGeom[i][0] << ", " << m_markerGeom[i][1] << ", "
                << m_markerGeom[i][2];
        for (int j = 0; j < m_nPoints; j++) {
            m_markerDiffs(i, j) = (m_markerGeom[i] - m_markerGeom[j]).norm();
            // Also save the greatest distance
            if (m_markerDiffs(i, j) > m_maxPointDistance)
                m_maxPointDistance = m_markerDiffs(i, j);
        }
    }
...
void PoseTracker::update(std::unique_ptr<std::vector<uint8_t>> irImLeft,
                         std::unique_ptr<std::vector<uint8_t>> irImRight)
{
    // Launch the keypoint search asynchronously and store the future to check later
    // auto fut = std::async(std::launch::async, [this](const Isometry3d& T,
    // std::unique_ptr<std::vector<uint16_t>> ir, std::unique_ptr<std::vector<uint16_t>> depth) {
    // return m_irTracker->findKeypointsWorldFrame(std::move(ir), std::move(depth), T); },
    // world_T_cam, std::move(irIm), std::move(depthMap)); m_detectionQ.try_enqueue(std::move(fut));
...
void PoseTracker::setJumpSettings(bool filterJumps,
                                  const float jumpThresholdMetres,
                                  const int numFramesUntilSet)
{
...
void PoseTracker::detectionThreadFunction()
{
    ...
    while (m_runDetectionThread) {
        ...
        if (detec.irImLeft && detec.irImRight) {
            auto detection = m_irTracker->findKeypointsWorldFrame(std::move(detec.irImLeft),
                                                                  std::move(detec.irImRight));

            if (detection->points.empty()) {
                setRoi(m_roi[0] * MROI_SCALE_X,
                       m_roi[1] * MROI_SCALE_Y,
                       m_roi[2] * MROI_SCALE_X,
                       m_roi[3] * MROI_SCALE_Y,
                       m_roiBuffer);
...
bool PoseTracker::preprocessMarkerDetection(std::shared_ptr<IRSegmentation::IrDetection> detection)
{
    ...
    while (i < detection->points.size()) {
        ...
        if (absval(detection->points[i][2]) > OUTLIER_FAR_CLIP
            || absval(detection->points[i][2]) < OUTLIER_NEAR_CLIP
            || absval(absval(detection->markerDiameters[i]) - m_markerDiameter)
                   > MARKER_DIAMETER_ERROR_THRESHOLD) {
            if (m_logLevel == IRSegmentation::LogLevel::VeryVerbose)
                LOG << "Removing outlier point [" << detection->points[i][0] << ", "
                    << detection->points[i][1] << ", " << detection->points[i][2]
                    << "] with diameter " << detection->markerDiameters[i];
...
void PoseTracker::processMarkerDetection(std::shared_ptr<IRSegmentation::IrDetection> detection)
{
    ...
    // --------------------------------------------- If the ith element of idxs equals j, then the
    // ith measured point corresponds to the jth known point
    if (m_logLevel == IRSegmentation::LogLevel::VeryVerbose) {
        std::string s = "";
        int i = 0;
        for (const auto &v : detection->points) {
            s += std::to_string(v[0]) + ", " + std::to_string(v[1]) + ", " + std::to_string(v[2])
                 + "; Diameter: " + std::to_string(detection->markerDiameters[i]) + "\n";
            i++;
        }
        LOG << s;
    }
    ...
    // Check if we got enough inliers - if we have mostly outliers, we can't do anything else so
    // just return I've never seen this condition triggered
    ...
    if (success) {
        ...
        } else if (m_logLevel == IRSegmentation::LogLevel::Verbose)
            LOG << "SUCCESS: Euclidean Matching with error " << err;
    }
    ...
    if (!success) {
        ...
        if (mps.size() <= 5 && mps.size() > 2) {
            ...
            if (m_logLevel == IRSegmentation::LogLevel::Verbose) {
                if (!success)
                    LOG << "Failed brute force with error " << err;
                else
                    LOG << "SUCCESS: brute force with error" << err;
            }
        }
    }
        ...
        if (m_logLevel == IRSegmentation::LogLevel::Verbose) {
            if (!success)
                LOG << "Failed with error " << err
                    << " after filling in missing"; // Never happens with 0.005 thresh
            else
                LOG << "SUCCESS: Filled in missing with error " << err;
        }
    }
    ...
    if (success && m_filterJumps) {
        ...
        if (!success && m_logLevel == IRSegmentation::LogLevel::Verbose)
            LOG << "Detected illegal jump";
    }
...
void PoseTracker::setPoseFromResult(std::shared_ptr<IRSegmentation::IrDetection> mes,
                                    const std::vector<int> idxs)
{
    Eigen::Isometry3d p = transformFromIdxs(mes->points, idxs);

    // First order low-pass filter
    m_lastRot = slerp(m_lastRot, Eigen::Quaterniond(p.rotation()), 1 - m_smoothing);
    m_lastPos = m_lastPos * m_smoothing + p.translation() * (1 - m_smoothing);

    // Set the current pose in a thread-safe way
    {
        std::lock_guard<std::mutex> lock(m_poseMutex);
        m_objectPose.pose = m_objectPose.pose.fromPositionOrientationScale(m_lastPos,
                                                                           m_lastRot,
    ...
    int maxX = 0;
    int maxY = 0;
    std::vector<Eigen::Vector2i> markerImPoints;
    for (size_t j = 0; j < mes->imCoords.size(); j++) {
        if (idxs.size() > j && idxs[j] != -1) {
            if (mes->imCoords[j][0] < minX)
                minX = mes->imCoords[j][0];
            if (mes->imCoords[j][1] < minY)
                minY = mes->imCoords[j][1];
            if (mes->imCoords[j][0] > maxX)
                maxX = mes->imCoords[j][0];
            if (mes->imCoords[j][1] > maxY)
                maxY = mes->imCoords[j][1];
        }
    }

    if (minX == 0 && minY == 0 && maxX == m_width && maxY == m_height) {
        // Debug.Log("No inlying points found");
        //  Don't reset ROI here. It should only happen in one place, otherwise it gets out of hand.
        //  See the else for kps.Count < 0
    } else {
...
{
    Eigen::Vector4i newRoi = {minX, maxX, minY, maxY};
    ...
    Eigen::Vector2d newRoiCentre = {(minX + maxX) / 2.0, (minY + maxY) / 2.0};
    Eigen::Vector2d oldRoiCentre = {(m_roi[0] + m_roi[1]) / 2.0, (m_roi[2] + m_roi[3]) / 2.0};

    // Compute the velocity of the ROI
    auto t = time();
    auto dt = t - m_lastRoiTime;
    m_lastRoiTime = t;
    Eigen::Vector2d newVel = (newRoiCentre - oldRoiCentre) / dt;

    // Smooth the velocity a bit
    Eigen::Vector2d vel = m_roiSmoothing * m_roiVel + (1 - m_roiSmoothing) * newVel;

    // Extrapolate the predicted position of the next ROI, assuming velocity and framerate stay
    // constant with slight deceleration
    Eigen::Vector2d offset = dt * m_roiDeceleration * vel;
    setRoi(minX + offset[0], maxX + offset[0], minY + offset[1], maxY + offset[1], m_roiBuffer);
}

double PoseTracker::fillInMissing(std::vector<Eigen::Vector3d> &mes, std::vector<int> &idxs)
{
    ...
    {
        // Find the transform
        std::vector<Eigen::Vector3d> mesPts;
        std::vector<Eigen::Vector3d> expPts;
        for (size_t i = 0; i < idxs.size(); i++) {
            if (idxs[i] != -1) {
        ...
        Eigen::Isometry3d M = findTransformFromPoints(mesPts, expPts);
    ...
    } else if (nPos == 3) // All but 1 accounted for. Should be easy to find the fourth
    {
        ...
        if (missing != -1) {
            ...
            if (nUA == 0) {
                ...
                Eigen::Vector3d pt = multPoint3x4(M, m_markerGeom[missing]);
                mesPts.push_back(pt);
                mes.push_back(pt); // push_back to the referenced list, which affects the call to
                                   // ukf.OnNewPointSample
...
bool PoseTracker::tryEuclideanMatching(const std::vector<Eigen::Vector3d> &mesPts,
                                       std::vector<int> &idxs)
{
    ...
    if (!hasRedundant && numAssigned == m_nPoints)
        return true;
    return false;
}

std::vector<int> PoseTracker::euclideanVotingOutliers(const std::vector<Eigen::Vector3d> &mesPts)
{
    if (mesPts.size() == 0)
...
double PoseTracker::bruteForceMatch(const std::vector<Eigen::Vector3d> &mesPts,
                                    std::vector<int> &idxsWithOutliers,
                                    std::vector<int> &idxs,
                                    bool useFixedIdxs)
{
    size_t np = mesPts.size();
    ...
    for (const auto &perm : permutations) {
        ...
        // Calculate transform
        Eigen::Isometry3d M = findTransformFromPoints(mes, exp);
    ...
    for (size_t i = 0; i < idxsWithOutliers.size(); i++) {
        if (idxsWithOutliers[i] == -1)
            continue;
        if (bestPerm[j] < 4)
            idxsWithOutliers[i] = bestPerm[j];
        else
            idxsWithOutliers[i] = -1;
        j++;
    }
    return bestErr;
}

Eigen::Isometry3d PoseTracker::transformFromIdxs(const std::vector<Eigen::Vector3d> &mes,
                                                 const std::vector<int> &idxs)
{
    std::vector<Eigen::Vector3d> mesPts;
    std::vector<Eigen::Vector3d> expPts;
...
Eigen::Isometry3d PoseTracker::findTransformFromPoints(const std::vector<Eigen::Vector3d> &mes,
                                                       const std::vector<Eigen::Vector3d> &exp)
{
    std::vector<Eigen::Vector3d> _mes(mes.size());
    std::vector<Eigen::Vector3d> _exp(exp.size());

    // Find average of each cluster of points
    Eigen::Vector3d mesAvg(0, 0, 0);
    Eigen::Vector3d expAvg(0, 0, 0);
    for (size_t i = 0; i < mes.size(); i++) {
        mesAvg += mes[i];
        expAvg += exp[i];
    }
    ...
    Eigen::Matrix3d R = kabsch(_mes, _exp);
    Eigen::Vector3d t = mesAvg - (R * expAvg);

    // Create homogeneous transformation matrix
    Eigen::Matrix4d pse = Eigen::Matrix4d::Identity();
    ...
    return Eigen::Isometry3d(pse);
}

Eigen::Matrix3d PoseTracker::kabsch(const std::vector<Eigen::Vector3d> &mes,
                                    const std::vector<Eigen::Vector3d> &exp)
{
    ...
    // Find rotation matrix (orthogonal, determinant 1) to map pointsFrom to pointsTo with minimum
    // RMSE Assumes the centroid of r is at [0,0,0] Covariance H = A'*B
    Eigen::Matrix3d H = At * B;
...
bool PoseTracker::svd(const Eigen::Matrix3d &mat, Eigen::Matrix3d &U, Eigen::Matrix3d &V)
{
    // Calculate SVD
    auto jacobiSvd = Eigen::JacobiSVD<Eigen::Matrix3d>(mat,
...
bool PoseTracker::noBadJumps(const std::vector<Eigen::Vector3d> &mes, std::vector<int> idxs)
{
...
double PoseTracker::meanMatchError(const std::vector<Eigen::Vector3d> &mesPts,
                                   const std::vector<int> &idxs)
{
    // Make the lists of points in the new order
    std::vector<Eigen::Vector3d> exp;
...
Eigen::Vector3d PoseTracker::multPoint3x4(const Eigen::Matrix4d &M, const Eigen::Vector3d &v)
{
    return M.topLeftCorner<3, 3>() * v + M.topRightCorner<3, 1>();
}

Eigen::Vector3d PoseTracker::multPoint3x4(const Eigen::Isometry3d &M, const Eigen::Vector3d &v)
{
    return M.rotation() * v + M.translation();
}

Eigen::Quaterniond PoseTracker::slerp(const Eigen::Quaterniond &q1,
