...
bool SetupAndSegment::GPUSetup()
{
    ...
    if (cudaEnabDevCount) {
        ...
        if (cudaEnabDevCount > 1) {
            ...
            std::cout << "" << std::endl;
            std::cout << "Pleases Enter the ID Number Of One Of The Above Compatable Devices: "
                      << std::endl;
            int id_selected;
            std::cin >> id_selected;
    ...
    if (!GPU_Found) {
        std::cout << "You don't have any compatable CUDA device(s), please set this up"
                  << std::endl;
    }
...
bool SetupAndSegment::RealSenseSetup(int timeout)
{
    ...
    try {
        _realSense_config.enable_stream(RS2_STREAM_INFRARED,
                                        1,
                                        REALSENSE_WIDTH,
                                        REALSENSE_HEIGHT,
                                        RS2_FORMAT_Y8,
        ...
        _realSense_config.enable_stream(RS2_STREAM_INFRARED,
                                        ...
                                        RS2_FORMAT_Y8,
                                        REALSENSE_FPS);
        _realSense_config.enable_stream(RS2_STREAM_DEPTH,
                                        REALSENSE_WIDTH,
                                        REALSENSE_HEIGHT,
                                        RS2_FORMAT_Z16,
                                        REALSENSE_FPS);
    ...
    try {
        auto depth_units
            = _realSense_context.query_devices().front().query_sensors().front().get_option(
    ...
    try {
        ...
        if (depth_sensor.supports(RS2_OPTION_EMITTER_ENABLED)) {
            depth_sensor.set_option(RS2_OPTION_EMITTER_ENABLED,
                                    ENABLE_REALSENSE_LASER); // Enable or disable emitter
        }
        ...
        for (auto &sensor : sensors) {
            if (sensor.supports(RS2_OPTION_GAIN)
                && sensor.get_info(RS2_CAMERA_INFO_NAME) == std::string("Stereo Module")) {
...
void SetupAndSegment::setCameraIntrinsics(
...
void SetupAndSegment::setCameraBoundaries(int roiUpperRow,
                                          int roiLowerRow,
                                          int roiLeftCol,
                                          int roiRightCol,
                                          ...
                                          double farClipPlane)
{
    ...
    if (m_logLevel == LogLevel::VeryVerbose)
        LOG << "Top row " << m_depthCamRoiUpperRow << ", bottom row " << m_depthCamRoiLowerRow
            << ", left col " << m_depthCamRoiLeftCol << ", right col " << m_depthCamRoiRightCol
            << ", near " << m_depthNearClip << ", far " << m_depthFarClip;
}
...
std::shared_ptr<SetupAndSegment::IrDetection> SetupAndSegment::findKeypointsWorldFrame(
    std::unique_ptr<std::vector<uint16_t>> irIm, std::unique_ptr<std::vector<uint16_t>> depthMap)
{
    ...
    if (success) {
        /*cv::Mat outputImage;
        cv::drawKeypoints(im8b,keypoints,outputImage, cv::Scalar(0, 255, 0),
        cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS); cv::imshow("Contours on Cropped", outputImage);
        cv::waitKey(1);*/
        ...
        for (size_t i = 0; i < keypoints.size(); i++) {
            ...
            double depth_m = (double) depth
                             * m_depth_scale; // Converts depth (16 bit representation) to meters
    ...
    while (_realsenseRunFrameThread) {
        {
            ...
            // Converts left IR to vector representation //ToDO: Change this so I am not
            // initializing an std:;vector<uint16_t> on every iteration
            ...
            // Converts depth frame to vector representation //ToDO: Change this so I am not
            // initializing an std:;vector<uint16_t> on every iteration
            auto depth_data = reinterpret_cast<const uint16_t *>(depth_filtered.get_data());
            std::vector<uint16_t> depth_vector(depth_data,
                                               depth_data + (REALSENSE_HEIGHT * REALSENSE_WIDTH));
...
bool SetupAndSegment::getRealSenseData(rs2::frame &ir_frame_left,
                                       ...
                                       std::unique_ptr<std::vector<uint16_t>> &ir_ptr)
{
    ...
    if (!_realsenseFrameArrivedVar.wait_for(lock, std::chrono::milliseconds(_timeout), [this]() {
            return (!_irLeftFrameQueue.empty()) && (!_irRightFrameQueue.empty())
                   && (!_depthFrameQueue.empty()) && (!_depthFilteredPtrQueue.empty())
                   && (!_irLeftPtrQueue.empty());
        })) {
