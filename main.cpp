...
//*****************Init Vars****************
rs2::frameset frameset; //Holds RealSense Frame Data
	...
	if (GPU_check)
	{
		...
		if (RealSense_check)
		{
			...
			while (1)
			{
				...
				auto ir_data = reinterpret_cast<const uint16_t*>(ir_frame_right.get_data());
				...
				//	cv::circle(ir_mat_right, cv::point(coord[0], coord[1]), 3, cv::scalar(0, 0, 255), -1);
				//}

				//cv::imshow("keypoints", ir_mat_right);
