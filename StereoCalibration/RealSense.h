...
const int REALSENSE_RETRY
    = 100 / REALSENSE_THREAD_DELAY; // We restart the pipeline if it skips by this amount
...
public:
    ...
    //  @param filter_alpha sets alpha of temporal depth filter used on depth vector passed to
    //  segmentation
    //  @param filter_delta sets delta of temporal depth filter used on depth vector passed to
    //  segmentation
    bool RealSenseInit(int width,
                       int height,
                       int fps,
                       float enable_laser,
                       float laser_power,
    ...
    rs2_intrinsics _realSense_intrinsics_leftIR,
        _realSense_intrinsics_rightIR; // Factory set intrinsics
...
private:
    ...
    //************Private Class Vars**************
    int _timeout; // Amount of time we wait for the thread to update a new frame before returning
                  // empty frames
